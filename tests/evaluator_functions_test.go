...
func TestEvaluateMetaFunctionCall(t *testing.T) {
    obj, _ := Evaluate(NewReaderWithPosition(strings.NewReader("meta(12)")), BuiltInScope)
    if !compareObjects(obj, NumberMetaObject) { t.Errorf("%v != %v.", obj, NumberMetaObject) }

    obj, _ = Evaluate(NewReaderWithPosition(strings.NewReader("meta(12.3)")), BuiltInScope)
...
func TestEvaluateSimpleFunctionDefinitionAndCall(t *testing.T) {
    obj, _ := Evaluate(NewReaderWithPosition(strings.NewReader("f(x, y) -> x + y\nf(1,1)")), BuiltInScope)
